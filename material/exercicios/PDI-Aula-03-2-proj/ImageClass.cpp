...
ImageClass::ImageClass()
{
    ...
    // NOVO
    // Isto resolve o problema de ter a imagem com largura m�ltipla de 4
...
bool ImageClass::Load(string nome)
{
    ...
    {
        if (data)
            free(data); // � necess�rio desalocar a �rea da imagem
...
void ImageClass::LoadFrom(string diretorio, string nome)
{
    ...
    if (!r)
    {
        printf("Imagem %s n�o carregada. Verifique o nome do Arquivo ou seu conteudo.", nome.c_str());
...
bool ImageClass::Inside(int x, int y)
{
    return x>=0 and y>=0 and x<this->getSizeX() and y<this->getSizeY();
}
